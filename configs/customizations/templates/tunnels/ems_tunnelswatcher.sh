...
for line in `cat /srv/configs/customizations/$customer/tunnels/mappings_ssh`; do
		...
		RCTESTTUNNEL=$?
			if [ $ISTUNNELUP == $LOCAL_PORT ] && [ $RCTESTTUNNEL == "0" ]
				then
				echo "Tunnel for $COMMON_NAME on $LOCAL_PORT is already up " >> /var/log/$customer_Tunnel_ems.log
 			else 
				...
				echo "Tunnel for $COMMON_NAME on $LOCAL_PORT has now been built " >> /var/log/$customer_Tunnel_ems.log
			fi
done
